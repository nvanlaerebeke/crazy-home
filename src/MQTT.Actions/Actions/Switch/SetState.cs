...
internal sealed class SetState {
    ...
    public SetState(
        MqttClient client, SwitchCache cache, HomeDbContextFactory dbContextFactory, ILogger<SetState> logger
    ) {
        _client = client;
        _cache = cache;
    ...
    private async Task CheckStateChanged(MqttApplicationMessageReceivedEventArgs eventArgs) {
        ...
        var switchStatus = JsonSerializer.Deserialize<SwitchStatus>(payload);
        if (switchStatus is null) {
            return;
        }

        //Plug state was not changed to ON yet
        if (!switchStatus.State.Equals(_switchState.ToString(), StringComparison.CurrentCultureIgnoreCase)) {
            return;
        }
        ...
        _cache.Set(switchStatus.ToDto(device));
        await _cancellationTokenSource.CancelAsync();
    }
}
